...
class MaskingAuditAndImportTests(unittest.TestCase):
    ...
    def test_account_get_does_not_return_password_or_refresh_token(self):
        ...
        try:
            cur = conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (email_addr, password, client_id, refresh_token, self._default_group_id(), "", "active"),
            )
    ...
    def test_system_group_cannot_be_renamed(self):
        ...
        resp = client.put(
            ...
            json={"name": system_group_name + "_new", "description": "x", "color": "#111111"},
        )
    ...
    def test_batch_import_returns_line_errors_and_parses_refresh_token_with_delimiter(self):
        client = self.app.test_client()
        self._login(client)
        ...
        resp = client.post(
            "/api/accounts",
            json={"account_string": account_string, "group_id": self._default_group_id()},
        )
        self.assertEqual(resp.status_code, 200)
        data = resp.get_json()
        self.assertEqual(data.get("success"), True)
    ...
    def test_audit_logs_endpoint_includes_group_create(self):
        ...
        create = client.post(
            "/api/groups",
            json={"name": group_name, "description": "", "color": "#222222", "proxy_url": ""},
        )
        ...
        matched = [
            r
            for r in logs
            if str(r.get("resource_id")) == str(group_id) and group_name in (r.get("details") or "")
        ]
